...
static fru_field_t *fru_encode_6bit(const unsigned char *s /**< [in] Input string */)
{
    ...
    if (len6bit > FRU_FIELDDATALEN(len6bit) ||
        !(out = (fru_field_t *)calloc(1, outlen)))
    {
        return out;
    }
...
fru_field_t * fru_encode_data(int len, const uint8_t *data)
{
    ...
    if (FRU_ISTYPE(typelen, ASCII_6BIT)) {
        ...
        if (!(out = (fru_field_t *)malloc(FRU_FIELDSIZE(typelen) + 1))) // Plus 1 byte for null-terminator
            return NULL;
                                      ...
                                      const unsigned char *strings[]) ///<[in] Array of strings for mandatory fields
{
    int field_count = 0;
    int padding_size;
    // Allocate the biggest possible header
    fru_board_area_t header;
    memset(&header, 0, sizeof(header));
    header.ver = FRU_VER_1;
    ...
    if (FRU_AREA_HAS_DATE(atype)) {
        uint32_t fru_time;
        struct tm tm_1996;
        memset(&tm_1996, 0, sizeof(tm_1996));
        tm_1996.tm_year = 96;
        tm_1996.tm_mon = 0;
        tm_1996.tm_mday = 1;
        const struct timeval tv_unspecified = {
            .tv_sec = 0,
            .tv_usec = 0,
        };
        struct timeval tv_1996;
        memset(&tv_1996, 0, sizeof(tv_1996));
        if (!tv) {
            errno = EFAULT;
            goto err;
        }
         ...
         */
        if (!memcmp(&tv_unspecified, tv, sizeof(struct timeval *))) {
    ...
    *outp = FRU_FIELD_TERMINATOR;
    outp += 1 + padding_size;
    *outp = fru_area_checksum((fru_info_area_t *)out);
...
int fru_mr_uuid2rec(fru_mr_rec_t **rec, const unsigned char *str)
{
    ...
    mgmt = (fru_mr_mgmt_rec_t *)calloc(1, sizeof(fru_mr_mgmt_rec_t) + UUID_SIZE);
...
//    uuid.time_low = uuid.time_low;
//    uuid.time_mid = uuid.time_mid;
...
fru_mr_reclist_t *add_mr_reclist(fru_mr_reclist_t **reclist)
{
    ...
    rec = (fru_mr_reclist_t *)calloc(1, sizeof(*rec));
    if(!rec) return NULL;
...
fru_mr_area_t *fru_mr_area(fru_mr_reclist_t *reclist, size_t *total)
{
    ...
    area = (fru_mr_area_t *)calloc(1, *total);
    if (!area) {
        *total = 0;
        return NULL;
    }
...
{
    fru_t fruhdr;
    memset(&fruhdr, 0, sizeof(fruhdr));
    fruhdr.ver = FRU_VER_1;
    ...
    for(i = 0; i < FRU_MAX_AREAS; i++) {
        ...
        fru_info_area_t *data = (fru_info_area_t *)area[i].data;
    ...
    out = (fru_t *)calloc(1, FRU_BYTES(totalblocks));
    ...
    for(i = 0; i < FRU_MAX_AREAS; i++) {
        ...
        uint8_t blocks = area[i].blocks;
        uint8_t *data = (uint8_t *)area[i].data;
        int area_offset_index = area_offsets[atype];
        uint8_t *offset = (uint8_t *)&fruhdr + area_offset_index;
...
static int parse_chassis_info(uint8_t *fru_data)
{
	char * fru_area;
	/*
...
static int parse_board_info(uint8_t *fru_data)
{
	char * fru_area;
	/*
...
static int parse_product_info(uint8_t *fru_data)
{
	char * fru_area;
	unsigned int i;
...
void decodings_fru(char* buf, int size)
{
	fru_t *fru = (fru_t *)buf;
    ...
    if (fru->chassis)
    {
        ...
        parse_chassis_info(fru_data);
    }

    if(fru->board)
    {
        fru_data = (uint8_t *)&buf[fru->board * 8];
        parse_board_info(fru_data);
    }

    if(fru->product)
    {
        parse_product_info(fru_data);
